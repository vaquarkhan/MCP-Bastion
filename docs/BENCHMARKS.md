...
> **Shareable deck:** **[MCP-Security-Deck-v3.pdf](../MCP-Security-Deck-v3.pdf)** ([raw download](https://github.com/vaquarkhan/MCP-Bastion/raw/main/MCP-Security-Deck-v3.pdf)) — security + FinOps proof slides for evaluators. This page is the **reproducible** source of the numbers.
...
| **Injection efficacy (offline heuristics)** | Known jailbreak bypass set vs benign traffic (incl. exfil sinks + decode) | **`tests/test_injection_efficacy.py`**  -  target **100%** attack block, **0%** benign FP (regex + normalize only) |
...
| Metric | Target |
|--------|--------|
| Attack block rate | **100%** on the fixed adversarial set (letter-spaced ignore, disregard paraphrase, DAN, system tags, exfil sinks, base64/hex/URL obfuscation, …) |
