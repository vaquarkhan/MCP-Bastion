...
| **JSON-RPC deny codes** | **21** | Errors **-32001** through **-32027** in `mcp_bastion.errors` (3.0 adds **-32025** canary, **-32026** LLM scanner, **-32027** ATR rules). |
...
| External policy (OPA / Cedar) | `policy_engine` | Delegates allow/deny to **OPA** (`opa eval -f raw`) or **Cedar** (`cedar authorize --policies … --entities …` with `User` / `Action::"invoke"` / `Tool`). Set `fail_closed: true` to deny when the engine is unavailable. |
