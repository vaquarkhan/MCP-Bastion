...
**Current release pin:** Dockerfiles use `ARG BASTION_VERSION=3.0.1`, which installs `mcp-bastion-python==3.1.0` inside the proxy image at build time (with PyPI CDN retries). The dashboard image copies `src/` from the repo.
...
**Current release:** **3.0.1** (2026-07-12) - `pip install mcp-bastion-python==3.1.0`

```bash
docker pull ghcr.io/vaquarkhan/mcp-bastion-proxy:v3.1.0
docker run -p 8080:8080 ghcr.io/vaquarkhan/mcp-bastion-proxy:v3.1.0
