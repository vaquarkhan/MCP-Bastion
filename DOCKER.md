...
**Current release pin:** Dockerfiles use `ARG BASTION_VERSION=4.0.0`, which installs `mcp-bastion-python==4.0.0` inside the proxy image at build time (with PyPI CDN retries). The dashboard image copies `src/` from the repo.
...
**Current release:** **4.0.0** (2026-07-31) - `pip install mcp-bastion-python==4.0.0`
