...
**Current release pin:** Dockerfiles use `ARG BASTION_VERSION=5.1.0`, which installs `mcp-bastion-python==5.1.0` inside the proxy image at build time (with PyPI CDN retries). The dashboard image copies `src/` from the repo.
...
**Current release:** **5.1.0** (2026-08-09) - `pip install mcp-bastion-python==5.1.0`

```bash
docker pull ghcr.io/vaquarkhan/mcp-bastion-proxy:v5.1.0
docker run -p 8080:8080 ghcr.io/vaquarkhan/mcp-bastion-proxy:v5.1.0
