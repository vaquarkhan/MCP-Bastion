...
**Current release pin:** Dockerfiles use `ARG BASTION_VERSION=3.3.0`, which installs `mcp-bastion-python==3.3.0` inside the proxy image at build time (with PyPI CDN retries). The dashboard image copies `src/` from the repo.
...
**Current release:** **3.3.0** (2026-07-21) - `pip install mcp-bastion-python==3.3.0`

```bash
docker pull ghcr.io/vaquarkhan/mcp-bastion-proxy:v3.3.0
docker run -p 8080:8080 ghcr.io/vaquarkhan/mcp-bastion-proxy:v3.3.0
